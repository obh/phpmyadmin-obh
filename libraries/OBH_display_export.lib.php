...
<form method="post" action="OBH_export.php" name="dump" >
...
if ($export_type == 'server') {
    ...
    echo '<input type="hidden" name="routine_name" value="'.$routine.'" />';
    echo PMA_generate_common_hidden_inputs($db,'',1);    
}
else {
...
<div class="exportoptions" id="header">
    <h2>
        ...
        if($export_type == 'server') {
            ...
            printf(__('Exporting procedure "%s"'),htmlspecialchars($routine));
        }
        else {
        ...
        <li>
            ...
            <ul id="ul_save_asfile">
                ...
                <li>
                    ...
                    <?php
                   // if($export_type=='procedure'){ $export_type='database'; }
                    echo __('File name template:');
                    $trans = new PMA_Message;
                    $trans->addMessage(__('@SERVER@ will become the server name'));
                    if ($export_type == 'database' || $export_type == 'table' || $export_type=='procedure') {
                        $trans->addMessage(__(', @DATABASE@ will become the database name'));
                        if($export_type=='procedure'){
                        $trans->addMessage(__(', @PROCEDURE@ will become the procedure name'));
                        }                        
