...
} elseif ($export_type == 'procedure' ) {
    //echo $routine;
    $err_url = 'db_proc_export.php';
...
if (!$save_on_server) {    
    if ($asfile) {
        ...
        } elseif($export_type == 'procedure' && isset($_REQUEST['procedure_select'])) {
            $_REQUEST['procedure_select'] = implode(",", $_REQUEST['procedure_select']);
            $procedure_select=$_REQUEST['procedure_select'];
        }
...
if ($export_type == 'server') {
    ...
    //echo "<br/>Mime Type ".$mime_type;
    //echo '<br/>FileName '.$filename;
    //exit();
    //OBH  $procedure_select contains all procedures which need to be
    //OBH  modify PMA_exportRoutines, pass another param $procedure_select and add an 'if' condition in the foor loop inside PMA_exportRoutines
    if (($GLOBALS[$what . '_structure_or_data'] == 'data' || $GLOBALS[$what . '_structure_or_data'] == 'structure_and_data') ) {
           foreach ($procedure_select as $procedure){
                $local_query  = 'SHOW CREATE PROCEDURE '.PMA_backquote($db).'.'.PMA_backquote($procedure) . ' ;';                
                if (!PMA_exportData($db, $table, $crlf, $err_url, $local_query)) {
                    break 2;
                }                
           }
    ...
    if (function_exists('PMA_exportRoutines')) {
        //&& strpos($GLOBALS['sql_structure_or_data'], 'structure') !== false && isset($GLOBALS['sql_procedure_function'])) {
            PMA_exportRoutines($db,$procedure_select);
    }    
    // OBH errror here sends back cannot find file 500 Internal Server Error
} else {
    if (!PMA_exportDBHeader($db)) {
        break;
    }
...
if ($save_on_server && isset($message)) {
    ...
    if ($export_type == 'server') {
        ...
        $active_page = 'db_proc_export.php';
        require './db_proc_export.php';
    } else {
        $active_page = 'tbl_export.php';
        require './tbl_export.php';
    }
...
if (!empty($asfile)) {
    ...
    if ($save_on_server) {
        ...
        if ($export_type == 'server') {
            ...
            $active_page = 'db_prox_export.php';
            require_once './db_proc_export.php';
        } else {
            $active_page = 'tbl_export.php';
